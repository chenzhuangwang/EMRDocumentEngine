 ...
 */
export function cumulativeCharWidths(text: string, config: CharWidthConfig): number[] {
  if (!text) return []
  const result: number[] = []
  let cum = 0
  for (const char of text) {
    cum += textMeasurer.measure(char, config).width
    result.push(cum)
  }
  return result
}
 ...
 */
export function cumulativeWidthUpTo(text: string, offset: number, config: CharWidthConfig): number {
  if (offset <= 0 || !text) return 0
  const cum = cumulativeCharWidths(text, config)
