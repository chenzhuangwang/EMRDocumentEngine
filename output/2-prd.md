...
- F13.5 编辑锁（Phase 1 独占编辑锁: Redis SET NX EX 30s 心跳续期, 架构 §12.1.2; **Phase 2 Yjs 上线后废弃, CRDT 自动合并替代互斥锁**）
...
**权威单一来源见架构 §17.1。** 此处不重复定义数字，仅列关键目标供快速参考：
...
| 连续打字帧率 (P95) | > 30fps | > 55fps | 架构 §17.1 |
| 100 页全量分页 | < 3s | < 1s | 架构 §17.1 |
| 10 页文档内存 | < 80MB | < 50MB | 架构 §17.1 |

完整指标表（含测量方法、前置依赖、4K 屏预算分析）见架构 §17.1。
